...
submit();eq(q('error').hidden,false,'No implicit timezone');
fill('zone','korea','change');submit();eq(q('error').hidden,false,'No implicit correction');
fill('clock','standard','change');submit();eq(q('error').hidden,false,'No implicit day boundary');
fill('boundary','zi23','change');submit();eq(q('error').hidden,true);
