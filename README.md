...
- 60년 / 60개월 / 60일 주기 선택, 같은 간지를 세 주기의 입춘·입추 기준점으로 공유
- 월·일 주기는 표시 시작 월·날짜를 바꿀 수 있고, 각 절기에 두 번의 연·월 또는 날짜 표시
...
- 자연순환 계산은 사용자가 정한 **60년·60개월·60일 / 24절기 / 한 절기 2.5년·2.5개월·2.5일** 규칙입니다. 표의 절기명은 이 주기의 상대적 위치이며 실제 천문 절입 날짜를 뜻하지 않습니다.
...
- 60개월 주기의 연·월은 **해당 절월 간지가 시작되는 양력 월**입니다. 시작 월부터 120개월 안의 두 연·월을 표시하며, 둘 사이 간격은 60개월(5년)입니다. 월 간지는 기존 사주 계산과 같은 절입 기준을 사용합니다. `+0.5개월`은 상대적 반달 표시이며 15일 등 임의의 날짜로 바꾸지 않습니다.
- 60일 주기는 표시 시작일부터 120일 안에 있는 두 양력 일진 날짜를 표시합니다. 둘 사이 간격은 60일이며 `+0.5일`은 해당 간지 기준점에서 반일(12시간) 뒤입니다. 날짜 표는 정확한 시각표가 아니며 자정 전후의 경계는 선택한 날짜 변경 기준을 따릅니다.
- 월·일 표시 시작값은 한국 시간의 이번 달·오늘이며 직접 변경할 수 있습니다(1900~2100년). 표시 기간은 생년월일이나 기준 간지를 바꾸지 않습니다. 적합도 선택은 주기별로 유지되고 생년월일을 바꾸거나 다시 계산하면 초기화됩니다.
...
- `src/lib/manse/period-cycles.js`: 같은 간지 기준의 60개월·60일 사이클 및 실제 연·월·날짜 대응
