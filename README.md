...
- `이 기기에 저장`으로 이름·입력한 양력/음력/윤달·성별·출생 시간·계산 기준·명식별 입춘/입추 선택 보관
- 저장한 사람 이름 검색, 불러오기, 수정 내용 저장, 새 항목으로 저장, 삭제 및 마지막 삭제 되돌리기
...
- 일주·시주 시간 기준은 당시 표준시(서머타임 제외), 날짜 변경 기준은 23시부터 다음 날을 기본 선택
...
- 입춘 또는 입추를 선택한 뒤 `오늘의 절기 보기`를 누르면 같은 간지 기준의 오늘 연·월·일 절기를 세 칸으로 표시
- 오늘 결과를 확인한 뒤 입춘·입추 선택을 바꾸면 세 결과를 함께 갱신하며, `아직 판단 불가능` 선택 시에는 결과를 숨김
- 기존 간지 직접 선택 화면 유지: `/manual`

결과 아래 적합도 선택 항목은 처음에 비어 있습니다. 출생 입력을 바꾸거나 다시 계산하면 이전 선택을 초기화합니다. 이름만 수정할 때는 계산과 선택을 유지합니다. 저장할 때는 이름이 필요하고 일반 계산에서는 선택 사항입니다.

저장 버튼을 누른 정보만 현재 사이트의 **같은 브라우저** `localStorage`에 보관합니다. 자동 저장·서버 전송·계정 동기화는 하지 않습니다. 시크릿 모드 종료, 브라우저 사이트 데이터 삭제 또는 브라우저의 데이터 정리로 목록이 사라질 수 있습니다. 다른 기기·브라우저·도메인과 목록이 공유되지 않으며 **미리보기와 운영 사이트의 저장 목록도 별개**입니다.

불러올 때 원래 입력한 생일 기준(한국 음력·윤달 포함)과 시간 설정을 복원해 사주를 다시 계산합니다. 선택은 명식의 사주팔자와 기준 간지가 일치할 때만 복원합니다. `오늘의 절기 보기`를 적용하고 저장한 항목은 불러온 현재 시각으로 다시 계산하며 과거의 오늘 결과를 재사용하지 않습니다. 변경한 입력과 선택은 저장 버튼을 눌러야 반영됩니다.

같은 이름도 고유 항목으로 구분합니다. 불러온 항목은 `수정 내용 저장`으로 갱신하고 `새 항목으로 저장`으로 복사할 수 있습니다. 마지막 삭제는 해당 페이지를 닫기 전까지 되돌릴 수 있습니다. 저장 공간 부족·접근 차단 시 성공으로 표시하지 않고 기존 정보를 유지합니다. 읽을 수 없는 항목은 삭제하지 않고 알리며, 다른 탭에서 이미 갱신된 항목은 다시 불러오도록 안내합니다.
...
- 오늘의 절기는 **60년·60개월·60일 주기**에서 현재 시각이 속한 2.5단위 구간을 표시합니다. 월·일 전체 표와 기간 선택란은 제공하지 않습니다.
- 오늘 계산의 경계 방식은 실행 버튼 앞에 표시합니다. 연은 실제 입춘부터 다음 입춘, 월은 실제 절입부터 다음 절입 사이의 경과 비율을 사용하고, 각 기간의 절반이 `+0.5`입니다. 일은 선택한 시간 보정과 날짜 변경 기준을 사용하며 반일은 12시간입니다. 이 추가 경계 방식을 확인한 뒤 `오늘의 절기 보기`를 눌러 적용합니다. 천문학상의 오늘 절기나 원광의 운세 해석을 뜻하지 않습니다.
- 출생 시각 대신 현재 시각으로 연·월·일 간지를 계산합니다. 시각 표시는 선택한 시간대이며, 화면의 기준 시각과 함께 1분마다 갱신하고 창으로 돌아오거나 새로고침 버튼을 눌러도 갱신합니다. 입력을 수정하거나 다시 계산하면 기존 오늘 결과를 지웁니다.
...
- `src/components/manse/saved-profiles.js`: 저장 목록·검색·수정·삭제·되돌리기 화면 동작
...
- `src/lib/manse/current-cycles.js`: 선택한 기준에 따른 현재 연·월·일 자연순환 절기
- `src/lib/manse/profile-store.js`: 브라우저의 사람별 저장 정보 검증과 읽기·쓰기
